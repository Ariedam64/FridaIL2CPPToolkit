 ...
 * event per message. Pass `sendClass` / `sendMethod` to target your game's protobuf send method.
 * Each event carries the IMessage class name and a ToString() preview of the payload.
 */
export function startNetworkCapture(sendClass: string, sendMethod: string): Promise<string> {
    return inVm(() => {
        ...
        for (const asm of Il2Cpp.domain.assemblies) {
            try {
                for (const k of asm.image.classes) {
                    ...
                    if (!getter) continue;
                    try {
                        // Force class static constructor to run (some protocol types
