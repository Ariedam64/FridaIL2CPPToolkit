...
exports.openClassDetail = openClassDetail;
const vscode = __importStar(require("vscode"));
async function openClassDetail(obfClassName, rpc, profileSource) {
    const panel = vscode.window.createWebviewPanel("fridaClassDetail", `Class: ${obfClassName}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    panel.iconPath = new vscode.ThemeIcon("symbol-class");
    let dump = "";
    try {
        dump = await rpc.call("dumpClassAsString", [obfClassName]);
    }
    catch (err) {
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.type) {
            ...
            case "traceMethod":
                if (typeof msg.methodName === "string") {
                    await vscode.commands.executeCommand("frida.hooks.traceMember", obfClassName, msg.methodName);
                }
                break;
            case "copySignature":
    ...
    if (profile) {
        const offLabel = profile.labels.onChange((e) => {
            if (matchesClass(e.key, obfClassName))
                render();
        });
...
function matchesClass(k, className) {
...
// HTML rendering
// ---------------------------------------------------------------------------
function renderClass(obf, dump, profile) {
    const labelKey = { kind: "class", className: obf };
...
function renderField(f, className) {
...
function renderMethod(m, className) {
...
//# sourceMappingURL=class-detail.js.map