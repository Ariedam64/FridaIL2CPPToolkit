...
export function openScanModal(): void {
    ...
    overlay.innerHTML = `
        <div style="background:var(--bg);border:1px solid var(--border-strong);border-radius:6px;padding:18px;width:780px;max-height:85vh;display:flex;flex-direction:column">
            ...
            <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px">
                ...
                <label style="font-size:11px">Class filter (regex, optional): <input class="ip-input" id="scan-classfilter" style="width:200px" placeholder="e.g. Inventory|Player"></label>
    ...
    async function runFirst(): Promise<void> {
        ...
        statusEl.innerHTML = `<span style="color:var(--warning)">Scanning all classes (this may take 30s+ on large games)...</span>`;
        firstBtn.disabled = true;
        try {
            const t0 = Date.now();
            const { matches } = await api.scanStart(value, { classFilter });
            const ms = Date.now() - t0;
            statusEl.textContent = `Found ${matches.length} match${matches.length === 1 ? "" : "es"} in ${ms}ms.`;
    ...
    async function runNext(): Promise<void> {
        ...
        try {
            ...
            const ms = Date.now() - t0;
            statusEl.textContent = `${matches.length} match${matches.length === 1 ? "" : "es"} after refine (${ms}ms).`;
            renderResults(matches);
            nextBtn.disabled = matches.length === 0;
        } catch (err) {
