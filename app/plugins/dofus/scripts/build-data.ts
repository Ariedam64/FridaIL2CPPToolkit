...
//   Fallback: ../catalog/maps.json  (already in OUT_DIR, generated by a
//             separate runtime scrape) — adds maps the canonical-coords sweep
//             didn't cover (instance dimensions, dungeons, special zones —
//             ~6.6k extras at last count). Without these the autopilot can't
//             walk on maps it knows are valid via world-graph.
...
function main(): void {
    ...
    // --- Fallback fill from catalog/maps.json ---
    // The catalog has every map the runtime knows about (~15k), including
    // instances/dungeons/special zones that canonical-coords didn't capture.
    // Anything still missing after the datacenter pass gets a row from here so
    // downstream code (DofusDataStore.loadMapDetail) won't bail on null-meta.
    let catalogAddedCount = 0;
    try {
        const catalogPath = path.join(OUT_DIR, "catalog", "maps.json");
        if (fs.existsSync(catalogPath)) {
            const catalog = JSON.parse(fs.readFileSync(catalogPath, "utf8")) as {
                items: Array<{ id: number; posX?: number; posY?: number; subAreaId?: number; worldMap?: number; nameId?: number; name?: string }>;
            };
            for (const m of catalog.items) {
                if (m.id == null || byMapId.has(m.id)) continue;
                byMapId.set(m.id, {
                    mapId: m.id,
                    posX: m.posX ?? 0,
                    posY: m.posY ?? 0,
                    ...
                    worldMap: m.worldMap ?? 0,
                    nameId: m.nameId ?? 0,
                    name: m.name && m.name.length > 0 ? m.name : `Map ${m.id}`,
                });
                catalogAddedCount++;
            }
        } else {
            console.warn(`[dofus:build-data] catalog/maps.json not found (skipping fallback fill)`);
        }
    } catch (err) {
        console.warn(`[dofus:build-data] catalog fallback failed:`, (err as Error).message);
    }
    ...
    console.log(
        `[dofus:build-data] wrote maps-information.json — ${mapsOut.length} maps (${enrichedCount} enriched from datacenter, ${catalogAddedCount} added from catalog)`,
    );
