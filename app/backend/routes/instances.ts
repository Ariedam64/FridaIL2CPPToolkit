...
export function mountInstances(app: Express, deps: InstancesDeps): void {
    ...
    app.post("/api/instances/scan/start", async (req, res) => {
        const { value, classFilter, maxMatches, skipFramework, typeFilter } = req.body ?? {};
        if (value === undefined || value === null) {
            res.status(400).json({ error: "value required" });
            return;
        }
        try {
            const matches = await deps.session.agentCall("valueScan", [value, { classFilter, maxMatches, skipFramework, typeFilter }]) as import("../core/instances/types.js").ScanMatch[];
