...
├── rpc-agent/  ← agent persistant exposant la lib via RPC (utilisé par le host)
└── presets/    ← configs par jeu cible (noms de classes, champs à surveiller)

host/
├── server.js   ← serveur Node qui pilote l'agent RPC et sert l'UI
└── public/     ← Operator Console (UI navigateur)
```

Deux flux d'utilisation cohabitent :

- **Operator Console (UI)** — recommandé. Tu lances `npm run host`, tu attaches au process via le navigateur, et tu pilotes search / hook / patch / scanner / inspector / diff / watchlist via une interface unique. Voir [Operator Console](#operator-console-ui).
- **Tools standalone (CLI)** — chaque `src/tools/*.ts` est un script Frida autonome avec un bloc `// === CONFIG ===` à éditer avant de builder. Plus brut, mais utile pour des hooks ciblés ou pour comprendre ce que la lib fait sous le capot.

## Operator Console (UI)

```bash
npm run build:all              # builds rpc-agent + tools + UI
npm run host                   # http://localhost:3000
```

Ouvre la page, choisis le process dans la sidebar, attache. À partir de là :

| Onglet | Rôle |
|---|---|
| `search` | Recherche regex de classes / champs / méthodes ; recherche de strings en mémoire ; dump de classe |
| `instance` | Capture d'instance (GC ou via hook), lecture/écriture de champs, appels de méthodes |
| `hook & patch` | Pose de hooks (avec capture de stack), `replaceNoop`, `forceReturn`, patch de statics |
| `scanner` | Scan de valeurs en mémoire (à la Cheat Engine) avec NEXT SCAN itératif |
| `inspector` | Arbre de champs primitifs / refs / List<T>, pin direct vers la watchlist |
| `diff` | Snapshot T0/T1 d'une instance ; les champs changés sont mis en évidence |
| `watchlist` | Pins live des champs surveillés ; copy-as-markdown pour partage |
| `bookmarks` | Sauvegarde de l'état (pins + hooks + patches) sous forme de presets par jeu |
| `socket` | Hook d'une méthode d'envoi protobuf et stream des messages sortants |
| `explorer` | Navigation par assembly / namespace / hiérarchie d'héritage |

Tous les dumps et bookmarks atterrissent sous `.toolkit-data/` (gitignoré).

## Workflow type (tools standalone)
